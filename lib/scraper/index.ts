  ...
  try {
    ...
    if (url.includes('lazada.')) {
      await page.waitForSelector(
        '.pdp-v2-product-price-content-salePrice-amount, .pdp-price_type_normal',
        { timeout: 10000 }
      );
      ...
      const lazadaPrices = await page.evaluate(() => {
        // --- Current Price ---
        let currentPrice =
          document.querySelector('.pdp-v2-product-price-content-salePrice-amount')?.textContent?.trim() || // new selector
          document.querySelector('.pdp-price_type_normal')?.textContent?.trim() || // old selector
          '';

        // --- Original Price ---
        let originalPrice =
          document.querySelector('.pdp-v2-product-price-content-originalPrice-amount')?.textContent?.trim() || // new selector
          document.querySelector('.pdp-price_type_deleted')?.textContent?.trim() || // old selector
          '';

        // --- Discount ---
        let discount =
          document.querySelector('.pdp-v2-product-price-content-originalPrice-discount')?.textContent?.trim() || // new selector
          document.querySelector('.pdp-product-price__discount')?.textContent?.trim() || // old selector
          '';
      ...
      const imageUrl = await page.evaluate(() => {
        const metaTag = document.querySelector('meta[property="og:image"]')?.getAttribute('content');
        const imgTag = document.querySelector('.gallery-preview-panel__image')?.getAttribute('src');
        return metaTag || imgTag || '';
      });
      ...
      console.log("✅ Lazada Prices:", lazadaPrices);
